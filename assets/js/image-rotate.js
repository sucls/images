...
function layoutImages(){
    ...
    let translateZOffset = 400// images[currIndex].clientHeight*0.8;
    let scaleOffset = 0.6;
    let rotateOffset = 360/images.length;
    let opacityOffset = 0.8;
    
    for(let i=0; i<images.length; i++){
        let offset = i - currIndex;
        let img = images[i];
        // let translate = offset*translateXOffset;
        let scale = scaleOffset**Math.abs(offset);
        let rotate =rotateOffset*(offset);
        console.log([rotate])
        img.style.transform = `rotateY(${rotate}deg) scale(0.8) translateZ(${translateZOffset}px)`;
